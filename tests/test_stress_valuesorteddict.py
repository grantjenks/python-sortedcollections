...
def test_init():
    ...
    sdict = ValueSortedDict((val, -val) for val in range(10000))
    sdict._check()
...
def stress_setdefault(sdict):
    ...
    for key in keys:
        if key in sdict:
            ...
            sdict.setdefault(key, -key)
            assert sdict[key] == -key
            del sdict[key]

def test_stress(repeat=10000):
    sdict = ValueSortedDict((val, -val) for val in range(1000))
